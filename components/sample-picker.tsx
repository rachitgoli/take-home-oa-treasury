...
export function SamplePicker({ onChoose, disabled }: Props) {
  ...
  async function choose(sample: SampleLabel) {
    ...
    try {
      ...
      onChoose(new File([blob], sample.file, { type: "image/png" }));
    } catch {
  ...
  return (
    <div>
      ...
      <p className="mt-1 text-sm text-zinc-600">
        Each one loads the matching application data too, so you can check it
        straight away.
      </p>
          ...
          <li key={sample.file}>
            ...
            >
              <span className="text-sm font-medium text-zinc-900">
                {loading === sample.file ? "Loading…" : sample.name}
              </span>
              <span className="text-xs text-zinc-600">{sample.expect}</span>
            </button>
          </li>
        ))}
      </ul>
